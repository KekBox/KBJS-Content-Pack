...
function SWEP:HolsterExtra()
	if IsValid(self.Owner) and self.Owner:GetNW2Bool("MeleeArtShieldening")==true then
		self.Owner:SetNW2Bool("MeleeArtShieldening",false)
...
function SWEP:Holster()
	if IsValid(self.Owner) and IsValid(self.Weapon) and CurTime() < self.NextAttack then
		return
	end

	if IsValid(self.Owner) and self.Owner:GetNW2Bool("MeleeArtStunned")==true then 
        return 
    end
	...
	self:HolsterExtra()
	self.Owner:SetNW2Bool("MeleeArtAttacking",false)
	self.Owner:SetNW2Bool("MeleeArtShoving", false)
	self.Owner:SetNW2Bool("MAGuardening", false)
	self.Owner:SetNW2Bool("MeleeArtThrowing",false)
	self.Charge2=0
	self.Charge=0
	if CLIENT and IsValid(self.Owner) and self.Owner:IsPlayer() then
	local vm = self.Owner:GetViewModel()
			...
			if (v.type == "Model" and IsValid(model)) then
				...
				if (v.surpresslightning) then
					...
					if name == "Laser" then
							...
							render.SetMaterial( redflare )
							render.DrawBeam(startpos, trc.HitPos, 0.2, 0, 0.99, Color(255,255,255, 100))
							render.DrawSprite( trc.HitPos,lsize,lsize,Color( 255,255,255 ) )
