				...
				if target:IsPlayer() then
					...
					if w == 1 then
						...
						if wepCheck then
							...
							ent:SetOwner(target)
							ent:SetNW2Int( 'throwdamage', 0 )
							ent:SetNW2Int( 'weaponname', wep.WepName )
							ent:SetNW2Int( 'impact1sound', wep.Impact1Sound )
							ent:SetNW2Int( 'impact2sound', wep.Impact2Sound )
							ent:SetNW2Int( 'hit1Sound', wep.Impact1Sound )
							ent:SetNW2Int( 'hit2Sound', wep.Impact2Sound )
							ent:SetNW2Int( 'hit3Sound', wep.Impact1Sound )
	...
	["expose"] = {
		...
		["function"] =
			function(target, attacker, effect, level)
				...
				target:SetNW2Bool("MAExpose",true)
				target:SetNW2Int( 'exposelevel', level )
			end
		},
	["bleed"] = {
		["string"] = "Bleed",
		["function"] =
			function(target, attacker, effect, duration, damage)
				...
				local timerID = "bleedTime" .. ((target:IsPlayer() and target:Name()) or target:EntIndex())
				if target:IsPlayer() then
				...
				timer.Create(timerID, 1, duration, function()
					if IsValid(target) and target:Alive() and target:Health()>1 then
						local d = DamageInfo()
						d:SetDamage( damage )
						...
						if GetConVarNumber( "ma2_togglebloodsplats" ) == 0 then
							return
						end
						local w = math.random(1)
						w = math.random(1,2)
